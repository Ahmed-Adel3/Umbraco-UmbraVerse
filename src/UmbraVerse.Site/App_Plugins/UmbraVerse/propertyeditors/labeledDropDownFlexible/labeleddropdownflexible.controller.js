angular.module("umbraco").controller("UmbraVerse.PropertyEditors.LabeledDropdownFlexibleController",
    function ($scope, validationMessageService) {
        ...
        function convertObjectToDictionaryArray(model){
            ...
            for (var i = 0; i < vals.length; i++) {
                var label = vals[i].label || vals[i].value || vals[i];
                var value = vals[i].value || vals[i];
                newItems.push({ 
                    id: keys[i], 
                    sortOrder: vals[i].sortOrder || 0, 
                    label: label,
                    value: value 
                });
            }
